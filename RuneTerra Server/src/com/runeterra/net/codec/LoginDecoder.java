package com.runeterra.net.codec;
...
import com.runeterra.RuneTerra;
import com.runeterra.net.ByteBufUtils;
import com.runeterra.net.NetworkConstants;
import com.runeterra.net.login.LoginDetailsMessage;
import com.runeterra.net.login.LoginResponses;
import com.runeterra.net.login.LoginUtils;
import com.runeterra.net.security.IsaacRandom;
...
public final class LoginDecoder extends ByteToMessageDecoder {
	...
	private void decodeRequest(ChannelHandlerContext ctx, ByteBuf buffer) {
		...
		if (request != NetworkConstants.LOGIN_REQUEST_OPCODE) {
			RuneTerra.getLogger().info("Session rejected for bad login request id: " + request);
	...
	private void decodeType(ChannelHandlerContext ctx, ByteBuf buffer) {
		...
		if (connectionType != NetworkConstants.NEW_CONNECTION_OPCODE
			...
			RuneTerra.getLogger().info("Session rejected for bad connection type id: " + connectionType);
			LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_SERVER_OFFLINE);
			return;
		}
	...
	private void decodeLogin(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) {
		...
		if (encryptedLoginBlockSize != buffer.readableBytes()) {
			RuneTerra.getLogger().info(String.format("[host= %s] encryptedLoginBlockSize != readable bytes",
		...
		if (buffer.isReadable(encryptedLoginBlockSize)) {
			...
			if (magicId != 0xFF) {
				RuneTerra.getLogger().info(String.format("[host= %s] [magic= %d] was rejected for the wrong magic value.",
			...
			if (memory != 0 && memory != 1) {
				RuneTerra.getLogger().info(String.format("[host= %s] was rejected for having the memory setting.",
						ctx.channel().remoteAddress()));
				LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_REJECT_SESSION);
				return;
			}
			...
			if (securityId != 10) {
				RuneTerra.getLogger().info(String.format("[host= %s] was rejected for having the wrong securityId.",
						ctx.channel().remoteAddress()));
				LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_REJECT_SESSION);
				return;
			}
