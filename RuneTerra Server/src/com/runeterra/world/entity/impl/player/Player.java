...
import com.runeterra.engine.task.impl.*;
...
import com.runeterra.world.model.*;
import com.runeterra.world.model.container.impl.*;
...
public class Player extends Character {
    ...
    public Character setHealth(int health) {
        ...
        skillManager.setCurrentLevel(Skill.HITPOINTS, health);
        packetSender.sendSkill(Skill.HITPOINTS);
        if (getHealth() <= 0 && !isDying) appendDeath();
        return this;
    }
    ...
    public void heal(int amount) {
        ...
        if ((skillManager.getCurrentLevel(Skill.HITPOINTS) + amount) >= level) {
            ...
            setHealth(skillManager.getCurrentLevel(Skill.HITPOINTS) + amount);
        }
    }
    ...
    public int getBaseAttack(CombatType type) {
        ...
        } else if (type == CombatType.MAGIC) return skillManager.getCurrentLevel(Skill.MAGIC);
        return skillManager.getCurrentLevel(Skill.ATTACK);
    }

    @Override
    public int getBaseDefence(CombatType type) {
        if (type == CombatType.MAGIC) return skillManager.getCurrentLevel(Skill.MAGIC);
    ...
    public void onLogin() {
        ...
        System.out.println("[World] Registering player - [username, host] : [" +
                                   getUsername() +
                                   ", " +
                                   getHostAddress() +
                                   "]");
        ...
        if (isNewPlayer()) {
            ...
            getPacketSender().sendMessage("Available commands:  ")
                    .sendMessage("")
                    .sendMessage("::item id amount")
                    .sendMessage("::setlevel skillId level")
                    .sendMessage("::master")
                    .sendMessage("::runes");
    ...
    public boolean busy() {
        return interfaceId > 0 || isDying || getHealth() <= 0 || isNeedsPlacement() || getStatus() != PlayerStatus.NONE;
    }
