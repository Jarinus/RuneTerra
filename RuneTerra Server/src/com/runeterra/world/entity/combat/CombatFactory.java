...
import com.runeterra.world.entity.combat.formula.DamageFormulas;
import com.runeterra.world.entity.combat.hit.HitDamage;
import com.runeterra.world.entity.combat.hit.HitMask;
...
import com.runeterra.world.model.movement.RS317PathFinder;
...
public class CombatFactory {
    ...
    public static boolean canReach(Character attacker, CombatMethod method, Character target) {
        ...
        if (attacker.isNpc()) {
            ...
            if (npc.getPosition().getDistance(npc.getSpawnPosition()) >=
                    npc.getDefinition().getCombatFollowDistance()) {
                npc.getCombat().reset();
    ...
    public static void handleQueuedHit(QueueableHit qHit) {
        ...
        if (attacker.isPlayer()) {
            ...
            if (Misc.getRandom(100) >= 90) {
                ...
                if (method.getCombatType() == CombatType.MELEE ||
                        p_.getCombat().getWeapon() == WeaponInterface.DART ||
    ...
    public static CombatMethod getMethod(Character attacker) {
        ...
        if (attacker.isPlayer()) {
            ...
            if (p.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 4747) {
                return new DualWieldCombatMethod();
            }
    ...
    public static boolean fullVeracs(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Verac the Defiled") :
    ...
    public static boolean fullDharoks(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Dharok the Wretched") :
    ...
    public static boolean fullKarils(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Karil the Tainted") :
    ...
    public static boolean fullAhrims(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Ahrim the Blighted") :
    ...
    public static boolean fullTorags(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Torag the Corrupted") :
    ...
    public static boolean fullGuthans(Character entity) {
        return entity.isNpc() ?
                entity.getAsNpc().getDefinition().getName().equals("Guthan the Infested") :
